...
class EvaluationSampleOutput(BaseModel):
    ...
    Attributes:
        input (EvaluationSampleInput): The original input data used
            for evaluation
        llm_response (str): The response generated by the language
            model entities
        (list[str]): List of entities extracted from the response
            accuracy (float):
        Accuracy score for the evaluation sample (0.0 to 1.0)
...
class CoverageMetric(Metric):
    ...
    concepts
    in the evaluation samples. It provides insights into how comprehensively
    the
...
class EvaluationResult(BaseModel):
    ...
    Attributes:
        ...
        individual sample
            evaluation results, containing the detailed outputs for each test
            case.
